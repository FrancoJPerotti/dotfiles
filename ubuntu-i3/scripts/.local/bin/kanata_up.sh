...
config="${HOME}/dotfiles/colemak-dh-ansi.kbd"
switcher="${HOME}/dotfiles/scripts/.local/bin/switch_layout.sh"
...
the fallback layout. Pass --ensure to guarantee the custom layout is active.
USAGE
        exit 0
        ;;
    *)
...
kanata_running=0
if pgrep -x "${kanata_name}" >/dev/null; then
    kanata_running=1
fi
